	...
	clusterify : function(nodes, clusterCount, interval, callback){
		...
		*/
		function groupClusters(nodes, centroids){
			var groups = [];
			for(let i = 0; i < centroids.length; i++) groups.push([]);
			
			for(let node of nodes) {
				let centIndex = getNearestCentroid(nodes, node, centroids);
				node.group = centIndex;
				groups[centIndex].push(nodes.indexOf(node));
			}	
